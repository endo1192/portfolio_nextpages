...
export function Post({ page, blocks }) {
  if (!page || !blocks) {
    return <div />;
  }
  return (
...
export const getStaticPropsNotion = async (textid: any) => {
  const { id } = textid;
  const page = await getPage(id);
  const blocks = await getBlocks(id);
